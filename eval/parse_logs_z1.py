...
def parse_log(path: Path, scenario: str, condition: str) -> RunResult:
    ...
    with path.open(encoding="utf-8", errors="replace") as fp:
        for raw in fp:
            ...
            # Cap parsing shortly after SIM_DURATION_S so runs that drifted
            # past the configured 30-min wall clock do not skew the
            # comparison.  We allow a 30-second slack so the BR_METRICS
            # line printed at exactly 30:00.7xx is still captured.
            # Cooja logs are time-ordered, so a break is safe.
            if ts is not None and ts > SIM_DURATION_S + 30:
                break
